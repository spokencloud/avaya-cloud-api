...
describe('RestClient Subscription Integration Test', () => {
  ...
  const integrationEnv = {
    user: 'yangtestadmin1',
    token:
      'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ5YW5ndGVzdGFkbWluMSIsImlzcyI6IkFCQ19TRUNVUklUWV9HQVRFV0FZIn0.I_-nlD5J3Jkdr-XihlD7rK-SjZgMqgI_soF2xHnAk34',
  ...
  test('getDataSubscription should retreive empty data', async () => {
    ...
    const subscriptions = await restClient.getDataSubscription(
      subAccountAppId,
  ...
  test('deleteDataSubscription should return 403 when subAccountAppId does not exists.', async () => {
    const testSubscriptionId = '200c216a-2178-44af-9927-ef28cb4307a7'
    const subscription = await restClient.deleteDataSubscription(
      'subaccount',
      testSubscriptionId
    )
  ...
  xtest('deleteDataSubscription should return 200 when subscription exists.', async () => {
    const testSubscriptionId = '27bd115d-3805-4019-ad30-e73c671af752'
    const subscription = await restClient.deleteDataSubscription(
      subAccountAppId,
      testSubscriptionId
    )
